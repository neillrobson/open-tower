/**
    This entire file comes directly from HaxeFlixel's FlxAssetPaths module.
...
class AssetLoader {
    public static function buildFileReferences(directory:String = "assets/",
            subDirectories:Bool = false, ?filterExtensions:Array<String>,
        ...
        var fileReferences:Array<FileReference> = getFileReferences(directory, subDirectories,
            filterExtensions, rename);
    ...
    static function getFileReferences(directory:String, subDirectories:Bool = false,
            ?filterExtensions:Array<String>, ?rename:String->String):Array<FileReference> {
        ...
        for (name in directoryInfo) {
            if (!FileSystem.isDirectory(resolvedPath + name)) {
                ...
                if (filterExtensions != null) {
                    var extension:String = name.split(".")
                        .pop(); // get the last string with a dot before it
